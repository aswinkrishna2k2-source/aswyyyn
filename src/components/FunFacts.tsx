...
function LogoDecor({ size = 114 }: { size?: number }) {
  return (
    ...
    >
      ...
      <mask id="ff-mask-2" maskUnits="userSpaceOnUse" x="0" y="27.2501" width="59" height="87" fill="black">
...
export default function FunFacts() {
  return (
    <section id="my-fun-facts" className="py-20 px-6 lg:px-16">
        ...
        <div className="flex items-start gap-10">
          <div className="flex flex-wrap gap-3 flex-1">
            {funFacts.map((fact) => (
              <span
                key={fact.id}
                className={`border border-white/15 px-4 py-2 text-muted ${font.label}`}
              >
                {fact.parts.map((part, i) =>
                  part.type === 'highlight' ? (
                    <span key={i} className="text-fg font-bold">
                      {part.value}
                    </span>
                  ) : (
